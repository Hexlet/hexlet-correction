...
public class CorrectionResourceIT {
    ...
    public static Correction createEntity(EntityManager em) {
        ...
        correction.setResolverRemark(DEFAULT_RESOLVER_REMARK);
        correction.setTextBeforeCorrection(DEFAULT_TEXT_BEFORE_CORRECTION);
        correction.setTextCorrection(DEFAULT_TEXT_CORRECTION);
        correction.setTextAfterCorrection(DEFAULT_TEXT_AFTER_CORRECTION);
        correction.setReporterName(DEFAULT_REPORTER_NAME);
        correction.setPageURL(DEFAULT_PAGE_URL);
        correction.setCorrectionStatus(DEFAULT_CORRECTION_STATUS);
        return correction;
    }
    ...
    public void createCorrection() throws Exception {
        final int databaseSizeBeforeCreate = correctionRepository.findAll().size();
    ...
    public void checkTextCorrectionIsRequired() throws Exception {
        final int databaseSizeBeforeTest = correctionRepository.findAll().size();
        // set the field null
    ...
    @Test
    @Transactional
    public void checkCorrectionStatusIsNotRequired() throws Exception {
        final int databaseSizeAfterTest = correctionRepository.findAll().size() + 1;
        // set the field null
        ...
        List<Correction> correctionList = correctionRepository.findAll();
        assertThat(correctionList).hasSize(databaseSizeAfterTest);
    }
    ...
    public void getAllCorrectionsByCommentsIsEqualToSomething() throws Exception {
        ...
        // Get all the correctionList where comment equals to commentsId
        defaultCorrectionShouldBeFound("commentsId.equals=" + commentsId);

        // Get all the correctionList where comment equals to commentsId + 1
    ...
    public void getAllCorrectionsByCorrecterIsEqualToSomething() throws Exception {
        // Initialize the database
        correctionRepository.saveAndFlush(correction);
        Preference correcter = PreferenceResourceIT.createEntity(em);
    ...
    public void getAllCorrectionsByResolverIsEqualToSomething() throws Exception {
        // Initialize the database
        correctionRepository.saveAndFlush(correction);
        Preference resolver = PreferenceResourceIT.createEntity(em);
        em.persist(resolver);
        em.flush();
    ...
    public void updateCorrection() throws Exception {
        ...
        updatedCorrection.setCorrecterRemark(UPDATED_CORRECTER_REMARK);
        updatedCorrection.setResolverRemark(UPDATED_RESOLVER_REMARK);
        updatedCorrection.setTextBeforeCorrection(UPDATED_TEXT_BEFORE_CORRECTION);
        updatedCorrection.setTextCorrection(UPDATED_TEXT_CORRECTION);
        updatedCorrection.setTextAfterCorrection(UPDATED_TEXT_AFTER_CORRECTION);
        updatedCorrection.setReporterName(UPDATED_REPORTER_NAME);
        updatedCorrection.setPageURL(UPDATED_PAGE_URL);
        updatedCorrection.setCorrectionStatus(UPDATED_CORRECTION_STATUS);
    ...
    public void deleteCorrection() throws Exception {
        ...
        final int databaseSizeBeforeDelete = correctionRepository.findAll().size();
